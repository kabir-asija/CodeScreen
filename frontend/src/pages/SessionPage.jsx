...
const SessionPage = () => {
  ...
  const { call, channel, chatClient, isInitializingCall, streamClient } =
    useStreamClient(sessionData, loadingSession, isHost, isParticipant);

  // find the problem data based on session problem title
  const problemData = sessionData?.problem
    ? Object.values(PROBLEMS).find((p) => p.title === sessionData.problem)
    : null;
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-base-content/60 mt-2">
                          Host: {sessionData?.host?.name || "Loading..."} •{" "}
                          {sessionData?.participant ? 2 : 1}/2 participants
                        </p>
                      </div>
